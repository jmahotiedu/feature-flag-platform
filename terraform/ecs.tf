...
resource "aws_lb" "main" {
  name               = "${substr(var.project_name, 0, 12)}-${var.environment}-alb"
...
resource "aws_lb_target_group" "ui" {
  name        = "${substr(var.project_name, 0, 12)}-${var.environment}-ui"
  port        = 80
  protocol    = "HTTP"
...
resource "aws_lb_target_group" "api" {
  name        = "${substr(var.project_name, 0, 12)}-${var.environment}-api"
  port        = 8080
  protocol    = "HTTP"
  vpc_id      = aws_vpc.main.id
  target_type = "ip"
...
resource "aws_ecs_task_definition" "ui" {
  ...
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.ui_container_cpu
  memory                   = var.ui_container_memory
  execution_role_arn       = aws_iam_role.ecs_task_execution.arn
  task_role_arn            = aws_iam_role.ecs_task_execution.arn
