...
TF_STATE_KEY="${TF_STATE_KEY:-feature-flag-platform/${TF_ENVIRONMENT}/terraform.tfstate}"
LOCK_TABLE_NAME="${LOCK_TABLE_NAME:-feature-flag-platform-tf-lock-${TF_ENVIRONMENT}}"
...
STATE_BUCKET="${STATE_BUCKET:-feature-flag-platform-tf-state-${AWS_ACCOUNT_ID}-${AWS_REGION}}"
...
if [[ "${APPLY}" == "true" ]]; then
  ...
  cd "${ROOT_DIR}"
  docker build -f Dockerfile.control-plane -t "${ECR_REGISTRY}/feature-flag-platform-api:${IMAGE_TAG}" .
  docker build -f Dockerfile.ui -t "${ECR_REGISTRY}/feature-flag-platform-ui:${IMAGE_TAG}" .
  docker push "${ECR_REGISTRY}/feature-flag-platform-api:${IMAGE_TAG}"
  docker push "${ECR_REGISTRY}/feature-flag-platform-ui:${IMAGE_TAG}"
  docker tag "${ECR_REGISTRY}/feature-flag-platform-api:${IMAGE_TAG}" "${ECR_REGISTRY}/feature-flag-platform-api:latest"
  docker tag "${ECR_REGISTRY}/feature-flag-platform-ui:${IMAGE_TAG}" "${ECR_REGISTRY}/feature-flag-platform-ui:latest"
  docker push "${ECR_REGISTRY}/feature-flag-platform-api:latest"
  docker push "${ECR_REGISTRY}/feature-flag-platform-ui:latest"
  cd "${TF_DIR}"
fi
